...
Este documento combina los dos laboratorios de VPN site-to-site (**IPsec + GRE** e **L2TP + IPsec**) sobre la misma topologia base, para tener toda la practica en un solo archivo. La configuracion base de cada router (hostname, AAA, ACL, NAT, DHCP) es identica en ambos metodos; solo cambia el bloque de VPN (seccion 5) y el nombre de la interfaz de tunel que reciben las rutas entre LANs.

**Como usar este documento:**
- Configura primero las secciones 1 a 4 (direccionamiento, ISP, Peer-A base, Peer-B base) igual para cualquiera de los dos metodos.
- En la seccion 5 elige **Opcion A (IPsec + GRE)** — la recomendada y funcional — o **Opcion B (L2TP + IPsec)** si tu profesor pide especificamente L2TP (ver aviso tecnico de plataforma abajo).
- Las secciones 6 en adelante (switches, Windows Server, Windows10-1, checklist) son comunes a ambas opciones.
...
| Tunel GRE (Opcion A) | 100.13.67.0/30 | Tu0 Peer-A=.1 / Tu0 Peer-B=.2 |
| Pool tunel L2TP (Opcion B) | 100.13.67.8/29 (.9-.14 asignables) | asignado por Peer-A via `ip local pool` |
| Credenciales VPN | usuario: **2025** / clave: **1367** | segun matricula 2025-1367 |

> IPsec PSK site-to-site no tiene un campo "usuario" nativo. En la Opcion A el **2025** de la matricula queda visible con `crypto keyring 2025` / clave `1367`. En la Opcion B queda visible con `local name 2025` y `l2tp tunnel password 1367` (VPDN).
...
## 4. Peer-B (R2 - lado Cliente, VLAN 20)
...
## 5. VPN Site-to-Site — elige una opcion

### Opcion A (recomendada) — IPsec + GRE
...
### Opcion B (alternativa / si el profesor exige L2TP) — L2TP + IPsec

> **Aviso tecnico importante:** en IOS clasico (12.4, feature-set `adventerprisek9`), el mecanismo `VPDN` para L2TP fue disenado para acceso remoto (dial-in de clientes PPP), no para tuneles router-a-router. La configuracion de esta seccion es el enfoque estandar mas cercano a "L2TP site-to-site" disponible en este feature-set (LNS/LAC con `vpdn-group` + `Virtual-Template`), pero en topologias GNS3 con c3725 puede no negociar la fase PPP de forma limpia sin un medio de marcado (dialer) real. Si al verificar no sube, es un limite de plataforma, no un error de sintaxis - documentalo asi ante tu profesor y ten `Laboratorio_VPN_IPsec_GRE.md` como la VPN funcional de respaldo.

**Bloque IPsec de transporte (protege el trafico L2TP/UDP 1701):**
...
**Tunel L2TP (VPDN) — Peer-A como LNS:**
...
**Comandos adicionales en Peer-B (LAC) para la Opcion B** (agregar despues del bloque base de Peer-B de la seccion 3, solo si eliges esta opcion):
...
**Verificacion especifica de la Opcion B:**
...
## 6. Switches (Switch1 - Servidor, Switch2 - Cliente)
...
## 7. Windows Server 2022 (AD DS, DNS, IIS, NPS) - Configuracion detallada
...
### 7.2 Promover el servidor a controlador de dominio
...
### 7.3 Verificar el rol DNS (se instala junto con AD DS)
...
### 7.4 Instalar el rol IIS (Servidor Web)
...
### 7.5 Crear la estructura de OUs, grupos y usuarios
...
### 7.7 Registrar los clientes RADIUS (los dos routers)
...
### 7.8 Crear las 3 directivas de red (una por nivel de privilegio)
...
### 7.9 Verificacion completa NPS + RADIUS de extremo a extremo
...
| DHCP para la LAN | Seccion 4; verificado en 8 |
| Ruta por defecto | Secciones 2, 3, 4 |
| NAT | Secciones 3, 4 |
| Autenticacion RADIUS | Secciones 3, 4, 7 |
| Comunicacion entre LANs solo por VPN | Seccion 5 (rutas via Tunnel0 o via tunel L2TP) |
| Solo HTTP+ICMP al servidor | ACL_HACIA_SERVIDOR, seccion 3; verificado en 8 |
| VPN Site-to-Site IPsec + GRE | Seccion 5, Opcion A |
| VPN Site-to-Site L2TP + IPsec | Seccion 5, Opcion B (ver aviso tecnico de plataforma) |
| ISP sin protocolo de enrutamiento | Seccion 2 |
| Windows Server: AD, DNS, IIS, NPS | Seccion 7 |
